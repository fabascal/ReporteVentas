    ...
    try {
      ...
      const response = {
        ...
        data: {
          ...
          agregados: {
            premium: {
              total_litros: parseFloat(datos.premium_volumen_total || '0'),
              total_importe: parseFloat(datos.premium_importe_total || '0'),
              precio_promedio: parseFloat(datos.premium_precio_promedio || '0'),
              total_merma_volumen: parseFloat(datos.premium_merma_volumen_total || '0'),
              total_merma_importe: parseFloat(datos.premium_merma_importe_total || '0'),
              merma_porcentaje_promedio: parseFloat(datos.premium_merma_porcentaje_promedio || '0'),
              total_eficiencia_real: parseFloat(datos.premium_eficiencia_real_total || '0'),
              total_eficiencia_importe: parseFloat(datos.premium_eficiencia_importe_total || '0'),
              eficiencia_real_porcentaje_promedio: parseFloat(datos.premium_eficiencia_real_porcentaje_promedio || '0')
            },
            magna: {
              total_litros: parseFloat(datos.magna_volumen_total || '0'),
              total_importe: parseFloat(datos.magna_importe_total || '0'),
              precio_promedio: parseFloat(datos.magna_precio_promedio || '0'),
              total_merma_volumen: parseFloat(datos.magna_merma_volumen_total || '0'),
              total_merma_importe: parseFloat(datos.magna_merma_importe_total || '0'),
              merma_porcentaje_promedio: parseFloat(datos.magna_merma_porcentaje_promedio || '0'),
              total_eficiencia_real: parseFloat(datos.magna_eficiencia_real_total || '0'),
              total_eficiencia_importe: parseFloat(datos.magna_eficiencia_importe_total || '0'),
              eficiencia_real_porcentaje_promedio: parseFloat(datos.magna_eficiencia_real_porcentaje_promedio || '0')
            },
            diesel: {
              total_litros: parseFloat(datos.diesel_volumen_total || '0'),
              total_importe: parseFloat(datos.diesel_importe_total || '0'),
              precio_promedio: parseFloat(datos.diesel_precio_promedio || '0'),
              total_merma_volumen: parseFloat(datos.diesel_merma_volumen_total || '0'),
              total_merma_importe: parseFloat(datos.diesel_merma_importe_total || '0'),
              merma_porcentaje_promedio: parseFloat(datos.diesel_merma_porcentaje_promedio || '0'),
              total_eficiencia_real: parseFloat(datos.diesel_eficiencia_real_total || '0'),
              total_eficiencia_importe: parseFloat(datos.diesel_eficiencia_importe_total || '0'),
              eficiencia_real_porcentaje_promedio: parseFloat(datos.diesel_eficiencia_real_porcentaje_promedio || '0')
            },
            totales: {
              total_ventas: parseFloat(datos.total_ventas || '0'),
              total_aceites: parseFloat(datos.aceites_total || '0'),
              dias_reportados: parseInt(datos.dias_reportados || '0'),
              gran_total: parseFloat(datos.total_ventas || '0') + parseFloat(datos.aceites_total || '0')
            }
          },
          metadata: {
  ...
  async getEficienciaEstaciones(req: Request, res: Response) {
    try {
      ...
      for (const zona of zonasResult.rows) {
        // Obtener estaciones de la zona con sus datos de eficiencia
        const estacionesResult = await pool.query(
          `SELECT 
            e.id, e.nombre, e.identificador_externo,
            rm.premium_eficiencia_real_total as premium_litros,
            rm.premium_eficiencia_real_porcentaje_promedio as premium_porcentaje,
            rm.magna_eficiencia_real_total as magna_litros,
            rm.magna_eficiencia_real_porcentaje_promedio as magna_porcentaje,
            rm.diesel_eficiencia_real_total as diesel_litros,
            rm.diesel_eficiencia_real_porcentaje_promedio as diesel_porcentaje,
            rm.dias_reportados,
        ...
        const estaciones = estacionesResult.rows.map(est => ({
          ...
          eficiencia: {
            premium: {
              litros: parseFloat(est.premium_litros || '0'),
              porcentaje: parseFloat(est.premium_porcentaje || '0')
            },
            magna: {
              litros: parseFloat(est.magna_litros || '0'),
              porcentaje: parseFloat(est.magna_porcentaje || '0')
            },
            diesel: {
              litros: parseFloat(est.diesel_litros || '0'),
              porcentaje: parseFloat(est.diesel_porcentaje || '0')
            }
          },
